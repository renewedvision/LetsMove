...
static NSString *ContainingDiskImageDevice(NSString *path) {
	...
	struct statfs fs;
	int err = statfs([containingPath fileSystemRepresentation], &fs);
	if (err)
		return nil;
	if (fs.f_flags & MNT_ROOTFS)
		return nil;
...
static BOOL Trash(NSString *path) {
	...
	NSLog(@"ERROR -- Could not trash '%@'. Error: %@", path, error.localizedDescription);
	return NO;
}
...
static BOOL DeleteOrTrash(NSString *path) {
	...
	NSLog(@"WARNING -- Could not remove '%@'. Error: %@", path, error.localizedDescription);
	NSLog(@"INFO -- Will attempt to trash");
